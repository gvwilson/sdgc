...
1.  Use `{% raw %}[Key1,Key2](BIB){% endraw %}` for bibliographic citations;
    the code in `assets/site.js` (which is automatically included and run in each page's footer)
...
1.  Use the following to include external figures:
    ...
    where the SVG file is in `figures/hyphenated-stem.svg`.
    The `explain` parameter should describe the image, not simply repeat the caption.
    Use `{% raw %}[f:slug:stem](FIG){% endraw %}` to refer to the figure,
    where `slug` is the lesson's slug.
